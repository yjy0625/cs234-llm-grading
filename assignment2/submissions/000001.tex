...
\vspace{-.5in}
\begin{framed}
{\bf Due date: Friday Jan 30, 2026 at 6:00 PM (18:00) PST}
\\[1em]
...
\\[1em]
Submissions are composed of 3 parts: Assignment 2 Written (PDF file), Assignment 2 Written (Tex file), and Assignment 2 Coding (.zip file). 
\end{framed}
...
%%%%% Start of 1(a) %%%%%
Answer: 
    Trivially, you won't need lines 1-3 because tabular Q learning doesn't require \textit{function approximation} and \textit{replay buffer} (data re-use).

    Here we're looking for key differences between DQN and tabular Q-learning, which are: 
    \begin{itemize}
    ...
    , so any lines with replay buffer (e.g., 10, 11); \textit{target network} (line 20); and \textit{function approximation} (SGD in line 18). Line 18 should be changed to the Bellman update: $$Q(s_t, a_t) \leftarrow Q(s_t, a_t) + \alpha \left[ r_t + \gamma \max_{a'} Q(s_{t+1}, a') - Q(s_t, a_t) \right]$$. 
%%%%% End of 1(a) %%%%%
\end{tcolorbox}
...
\begin{tcolorbox}[breakable]
%%%%% Start of 1(b) %%%%%
Answer: If the states and action spaces are extremely large (e.g., continuous valued states and actions).
%%%%% End of 1(b) %%%%%
\end{tcolorbox}
\clearpage 
...
\begin{tcolorbox}[breakable]
%%%%% Start of 1(c) %%%%%
Answer: DQN is an off-policy algorithm, so it can use dataset collected by a different policy. Without replay buffer (using on-policy data only), function approximation and Q learning update will both suffer from a lack of state and action coverage; alternatively, you will need to collect a large amount of on-policy data. Replay buffer increases data efficiency by incorporating more state and action tuples (from different policies) in the Q function update.
%%%%% End of 1(c) %%%%%
\end{tcolorbox}
\end{enumerate}
...
\begin{tcolorbox}[breakable]
%%%%% Start of 2(a) %%%%%
Answer: The key observation is that we can express the returns recursively:
\[G_t =
\begin{cases}
r_T & t = T\\
r_t + \gamma G_{t+1} & t < T
\end{cases}\]
Hence we can compute the returns backwards from $t = T$ down to $t = 0$. There are $O(T)$ values to
compute, and each can be computed in $O(1)$ time, for a total of $O(T)$.
%%%%% End of 2(a) %%%%%
\end{tcolorbox}
\clearpage 
...
\begin{tcolorbox}[breakable]
%%%%% Start of 2(b) %%%%%
Answer: 1. Whenever the clip term is activated, i.e., the policy is changing a lot from the previous policy and hence the value of $J_{\text{clip}}(\theta)$ is independent of $\theta$ in those cases as PPO only wants to take small steps in updating the policy. That is, if $z_\theta > 1 + \epsilon$ and $\hat{A} > 0$ or $z_\theta < 1 - \epsilon$ and $\hat{A} < 0$, the gradient is zero.

2. When the policy has converged to some local optima where $\nabla_{\theta}(\pi_{\theta}) = 0$.

3. When the advantage function $\hat{A}=0$. 
%%%%% End of 2(b) %%%%%
\end{tcolorbox}
\clearpage 
...
\begin{tcolorbox}[breakable]
%%%%% Start of 2(c) %%%%%
Answer: REINFORCE does not use the ``old" log probabilities (i.e., the log probabilities corresponding to the policy that was used to collect the updates) since the data is updated every iteration with the latest policy (it is on-policy). Since PPO uses the ratio of the ``current" policy and the "old" policy (which is used to collect data), we need to collect this information for PPO specifically.

If we did not collect this information, we could always compute it on the fly by storing and tracking the old policy. This incurs additional computational cost in terms of time (an additional forward pass producing the same output for $K$ updates) and memory (an additional policy to store).
%%%%% End of 2(c) %%%%%
\end{tcolorbox}
\clearpage
...
\begin{enumerate}
 ...
 \begin{tcolorbox}[breakable]
%%%%% Start of 3(a) %%%%%
Answer: $$\rho^\pi(\tau) = \prod\limits_{t=0}^\infty \pi(a_t|s_t) \mathcal{P}(s_{t+1}|s_t,a_t)$$
%%%%% End of 3(a) %%%%%
\end{tcolorbox}
\clearpage
...
\begin{tcolorbox}[breakable]
%%%%% Start of 3(b) %%%%%
Answer: $$  p^\pi(s_t = s) = \sum_{\tau_{t-1}} \rho^\pi(\tau_{t-1})\mc{P}(s \mid s_{t-1}, a_{t-1})$$
%%%%% End of 3(b) %%%%%
\end{tcolorbox}
\clearpage
...
%%%%% Start of 3(c) %%%%%
Answer: \begin{align*}
			\mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \gamma^t f(S_t,A_t)\right] 
			&= \sum\limits_{t=0}^\infty \gamma^t\mathbb{E}_{\tau \sim \rho^\pi}\left[ f(S_t, A_t)\right]\\
			&= \mathbb{E}_{\tau \sim \rho^\pi}\left[ f(S_0, A_0)\right] + \gamma \mathbb{E}_{\tau \sim \rho^\pi}\left[ f(S_1, A_1)\right] + 
				\gamma^2\mathbb{E}_{\tau \sim \rho^\pi}\left[ f(S_2, A_2)\right] + ...\\
			% &= \mathbb E_{S_0} \left[\sum_{a \in \mathcal A} \pi(a|S_0) f(S_0, a) + 
				% \gamma \sum_{a \in \mathcal A}\pi(a|S_0)\sum_{s' \in \mathcal S}\mathcal{P}(s'|S_0,a)\sum_{a' \in \mathcal A}\pi(a'|s')f(s', a') + ... \right]\\
			&= \sum_{s \in \mathcal S} \left[ p^\pi(S_0=s) \mathbb{E}_{A\sim\pi(s)}[f(s, A)]\right] + \gamma \sum_{s' \in \mathcal S} \left[p^\pi(S_1=s') \mathbb{E}_{A'\sim\pi(s')}[f(s', A')] \right] + ...\\
			&= \sum_{t=0}^{\infty} \left[
 \sum_{s \in \mathcal S} \gamma^t p^\pi(S_t=s)\mathbb{E}_{A\sim\pi(s)}[f(s, A)] \right] \\
			&= \frac{1}{(1-\gamma)}\sum_{s \in \mathcal S} d^\pi(s)\mathbb{E}_{A\sim\pi(s)}[f(s, A)] \\
...
%%%%% Start of 3(d) %%%%%
Answer: \begin{align*}
			V^\pi(s_0) - V^{\pi'}(s_0) &= \mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \gamma^t \mathcal{R}(s_t,a_t)\right] - V^{\pi'}(s_0) \text{ (Def. of policy value) } \\
			&= \mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \gamma^t \mathcal{R}(s_t,a_t) - V^{\pi'}(s_0) \right] \text{ (Linearity of expectation)}\\
			&= \mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \left(\gamma^t \left(\mathcal{R}(s_t,a_t) + \gamma V^{\pi'}(s_{t+1})\right)\right) - \sum\limits_{t=0}^\infty \gamma^{t+1} V^{\pi'}(s_{t+1}) - V^{\pi'}(s_0) \right] \text{ (Adding ``zero'')}\\
            &= \mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \left(\gamma^t \left(\mathcal{R}(s_t,a_t) + \gamma V^{\pi'}(s_{t+1})\right)\right) - \sum\limits_{t=0}^\infty \gamma^{t} V^{\pi'}(s_{t})\right]\\
			...
			&= \sum\limits_{t=0}^\infty \gamma^t\mathbb{E}_{\tau \sim \rho^\pi}\left[ \mathcal{R}(s_t,a_t) + \gamma V^{\pi'}(s_{t+1}) - V^{\pi'}(s_t)\right] \text{ (Linearity of expectation)} \\
			&= \sum\limits_{t=0}^\infty \gamma^t\mathbb{E}_{\tau_t \sim \rho^\pi}\left[\mathbb{E}_{s_{t+1}}\left[ \mathcal{R}(s_t,a_t) + \gamma V^{\pi'}(s_{t+1}) - V^{\pi'}(s_t) \Big| s_t, a_t \right]\right] \text{ (Tower property of expectation)} \\
			&= \sum\limits_{t=0}^\infty \gamma^t\mathbb{E}_{\tau_t \sim \rho^\pi}\left[ \mathcal{R}(s_t,a_t) + \gamma \mathbb{E}_{s_{t+1}}\left[V^{\pi'}(s_{t+1}) \Big| s_t, a_t \right] - V^{\pi'}(s_t)\right] \\
			&= \mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \gamma^t\left( \mathcal{R}(s_t,a_t) + \gamma\mathbb{E}_{s_{t+1}}\left[ V^{\pi'}(s_{t+1}) \Big| s_t, a_t\right] - V^{\pi'}(s_t)\right)\right] \\
			&= \mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \gamma^t\left( Q^{\pi'}(s_t,a_t) - V^{\pi'}(s_t)\right)\right] \\
			&= \mathbb{E}_{\tau \sim \rho^\pi}\left[\sum\limits_{t=0}^\infty \gamma^t A^{\pi'}(s_t,a_t)\right] \\
			&= \frac{1}{(1-\gamma)} \mathbb{E}_{s \sim d^\pi}\left[\mathbb{E}_{a \sim \pi(s)}\left[A^{\pi'}(s,a)\right]\right]. \text{ ( Use $f := A^{\pi'}$ in the result from (c))}
		\end{align*}
%%%%% End of 3(d) %%%%%
\end{tcolorbox}
\clearpage
...
\begin{tcolorbox}[breakable]
%%%%% Start of 4(a) %%%%%
Answer: I would suggest the following two experimental design choices: 1. Ensure that students are fully informed about the nature of the experiment and the potential risks and benefits of participating. This respects the principle of respect for persons, as it allows students to make informed decisions about their participation. 2. Implement a monitoring system to track the performance of the chatbot and intervene if it is found to be providing suboptimal advice that could negatively impact student grades. This aligns with the principle of beneficence, as it aims to minimize potential harm to students by ensuring they receive adequate support throughout the course.
%%%%% End of 4(a) %%%%%
\end{tcolorbox}
\end{enumerate}
...
\begin{tcolorbox}[breakable]
%%%%% Start of 4(b) %%%%%
Answer: To get clearance for conducting the above experiment at Stanford, you would need to submit a research protocol to Stanford’s Institutional Review Board (IRB) through their online submission system. You would typically start by contacting the Stanford Research Compliance office or the IRB office via email to get guidance on the submission process. The research protocol should include details about the study design, informed consent process, potential risks and benefits, and measures to protect participant privacy and confidentiality.
%%%%% End of 4(b) %%%%%
\end{tcolorbox}
\end{enumerate}
